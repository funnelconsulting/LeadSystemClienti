...
exports.saveLeadSMCNew = async (req, res) => {
  console.log(req.body);
  try {
    const {
      id,
...
exports.saveLeadLuissNew = async (req, res) => {
  console.log(req.body);
  try {
    const {
      id,
...
exports.saveLeadUnusualNew = async (req, res) => {
  console.log(req.body);
  try {
    const {
      id,
...
exports.saveLeadMira = async (req, res) => {
  console.log(req.body);
  try {
    const {
      id,
    ...
    //const specificField = custom_fields.find(field => field.id === '500329');
    const specificFieldApp = custom_fields.find(field => field.id === '940106');
    const linkChatField = custom_fields.find(field => field.id === "637949");
    const date = new Date(parseInt(last_interaction));
    const formattedDate = date.toLocaleString();
 ...
 if (lead) {
      ...
      console.log('Lead aggiornata con successo nel database!');
        ...
        const newLead = new Lead({
          ...
          esito: 'Da contattare',
          orientatori: null,
          utente: "66aa423c1c13e2b53b36c9f0", //"662f767d3eda57d593f420fe", TEST ACCOUNT
          note: '',
          fatturato: '',
          utmCampaign: 'AI chatbot',
          utmSource: canale || '',
        ...
        try {
          ...
          if (!existingLead) {
            ...
            await lead.save();
            await newLead.save();
            console.log(`Assegnato il lead ${lead.cognome} all'utente miraibay`);
            await user.save();
            //await createEvent("smc@scuolamotociclismo.com", newLead.appDate, nome + ' ' + cognome, conversation_summary, phone, email)
          } else {
            console.log(`Già assegnato il lead ${lead.cognome} all'utente miraibay`)
            if (!isValidPhoneNumber(phone)){
              console.log('Numero non valido')
              return
            }
...
exports.saveLeadVideoAnimate = async (req, res) => {
  console.log(req.body);
  try {
    const {
      id,
 ...
 if (lead) {
      ...
      await lead.save();
      console.log('Lead aggiornata con successo nel database!');
        const userId = '6703b24559fdcb414e62cda1'; //'662f767d3eda57d593f420fe'; TEST ACCOUNT
        let user = await User.findById(userId);
        const newLead = new Lead({
          data: new Date(),
          nome: first_name,
          ...
          esito: 'Da contattare',
          orientatori: null,
          utente: "6703b24559fdcb414e62cda1", //"662f767d3eda57d593f420fe", TEST ACCOUNT
          note: '',
          fatturato: '',
          utmCampaign: 'AI chatbot',
          utmSource: canale || '',
...
exports.saveLeadSunlight = async (req, res) => {
  console.log(req.body);
  try {
    const {
      id,
 ...
 if (lead) {
      ...
      await lead.save();
      console.log('Lead aggiornata con successo nel database!');
        const userId = '66e7f3058f70e667cb128461'; //'662f767d3eda57d593f420fe'; TEST ACCOUNT
        let user = await User.findById(userId);
        const newLead = new Lead({
          data: new Date(),
          nome: first_name,
          ...
          esito: 'Da contattare',
          orientatori: null,
          utente: "66e7f3058f70e667cb128461", //"662f767d3eda57d593f420fe", TEST ACCOUNT
          note: '',
          fatturato: '',
          utmCampaign: 'AI chatbot',
          utmSource: canale || '',
